...
function agregarEncabezadoCENSA(doc, tipoReporte) {
    ...
    doc.rect(0, 0, anchoTotal, 55, 'F');
    
    // Agregar logo a la izquierda
    try {
        doc.addImage(logoBase64, 'JPEG', 15, 8, 35, 35);
    } catch (error) {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('POLITECNICO NUESTRA SEÑORA DE LA ALTAGRACIA', centroX, 15, { align: 'center' });
    
    // Subtítulo en verde
    doc.setTextColor(verde[0], verde[1], verde[2]);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.text('MINERD-Red de Escuelas Salesianas (FMA)', centroX, 25, { align: 'center' });
    
    // Unidad de Gestión en negro
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('Unidad de Gestión de la Convivencia', centroX, 35, { align: 'center' });
    
    // Línea separadora azul
    doc.setDrawColor(azulOscuro[0], azulOscuro[1], azulOscuro[2]);
    doc.setLineWidth(0.5);
    doc.line(15, 48, anchoTotal - 15, 48);
    
    // Tipo de reporte en azul
    doc.setTextColor(azulOscuro[0], azulOscuro[1], azulOscuro[2]);
    doc.setFontSize(13);
    doc.setFont('helvetica', 'bold');
    doc.text(tipoReporte, centroX, 56, { align: 'center' });
    
    // Restablecer color de texto a negro
    doc.setTextColor(0, 0, 0);
    doc.setFont('helvetica', 'normal');
    
    return 63; // Posición Y donde comienza el contenido (aumentado para dar más espacio)
}
